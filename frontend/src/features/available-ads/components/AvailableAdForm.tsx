...
export function AvailableAdForm() {
  ...
  const handleExtract = async () => {
    setLoading(true);
    try {
      const data = await fetchApi<any>('/extraction/available/preview', {
        method: 'POST',
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow space-y-4">
      <h2 className="text-xl font-bold">Paste Available Rental Ad</h2>
      <textarea
        className="w-full p-2 border rounded font-mono text-sm"
        rows={6}
        placeholder="Paste raw ad text here..."
        value={rawText}
